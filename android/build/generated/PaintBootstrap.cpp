...
#define TAG "work.addiel.paint"

using namespace v8;

static Persistent<Object> bindingCache;

static void Paint_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Paint.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::PaintBindings::lookupGeneratedInit(
...
static void Paint_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Paint_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Paint_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::PaintBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry PaintBinding = {
	"work.addiel.paint",
	Paint_init,
	Paint_dispose
};
...
Java_work_addiel_paint_PaintBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("work.addiel.paint", &PaintBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::PaintBindings::lookupGeneratedInit));
}
